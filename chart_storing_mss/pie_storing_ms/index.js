...
const db = mongo_client.db('diagram_pie_chart');

const setupConsumer = () => {
    try {
        const consumer = new kafka.Consumer(
            client,
            [{ topic: 'svg-chart-pie', partition: 0 }],
            { autoCommit: true }
        );
        ...
        consumer.on('message', async function (message) {
            ...
            const result = await db.collection('diagram_pie_chart').insertOne(diagram);
...
app.get('/api/diagrams/:userID', async (req, res) => {
    ...
    const diagrams = await db.collection('diagram_pie_chart').find({ userID }).toArray();

    res.json(diagrams);
});

app.listen(port, () => {
    console.log(`Store microservice (pie-chart) is running at http://localhost:${port}`);
});