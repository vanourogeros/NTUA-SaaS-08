...
const db = mongo_client.db('diagrams_line_annotations');

const setupConsumer = () => {
    try {
        const consumer = new kafka.Consumer(
            client,
            [{ topic: 'svg-chart-line-annotations', partition: 0 }],
            { autoCommit: true }
        );
        ...
        consumer.on('message', async function (message) {
            ...
            const result = await db.collection('diagrams_line_annotations').insertOne(diagram);
...
app.get('/api/diagrams/:userID', async (req, res) => {
    ...
    const diagrams = await db.collection('diagrams_line_annotations').find({ userID }).toArray();

    res.json(diagrams);
});

app.listen(port, () => {
    console.log(`Store microservice (line-annotations) is running at http://localhost:${port}`);
});