...
const db = mongo_client.db('diagram_basic_column');
...
const setupConsumer = () => {
    try {
        ...
        consumer.on('message', async function (message) {
            ...
            const result = await db.collection('diagram_basic_column').insertOne(diagram);
...
app.get('/api/diagrams/:userID', async (req, res) => {
    ...
    const diagrams = await db.collection('diagram_basic_column').find({ userID }).toArray();
