...
const db = mongo_client.db('diagram_dependency_wheel');

const setupConsumer = () => {
    try {
        const consumer = new kafka.Consumer(
            client,
            [{ topic: 'svg-chart-dependency-wheel', partition: 0 }],
            { autoCommit: true }
        );
        ...
        consumer.on('message', async function (message) {
            ...
            const result = await db.collection('diagram_dependency_wheel').insertOne(diagram);
...
app.get('/api/diagrams/:userID', async (req, res) => {
    ...
    const diagrams = await db.collection('diagram_dependency_wheel').find({ userID }).toArray();

    res.json(diagrams);
});

app.listen(port, () => {
    console.log(`Store microservice (dependency-wheel) is running at http://localhost:${port}`);
});